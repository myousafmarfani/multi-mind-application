...
async def gemini_agent(prompt: str, instructions: str = "You are a helpful assistant. understand the user message context. if user is asking for explanations, provide detailed answers.", conversation_history: list = None, user_name: str = None) -> str:
    ...
    if conversation_history and len(conversation_history) > 0:
        print(f"Gemini Agent - Processing {len(conversation_history)} context messages")
...
Answer (as Gemini, based on the conversation context above):"""
        ...
        print(f"Gemini Agent - Using enhanced prompt with context")
        print(f"Gemini Agent - Enhanced prompt: {enhanced_prompt}")
    else:
        enhanced_prompt = f"{user_greeting}\n\n{prompt}"
        print(f"Gemini Agent - No conversation context available")
...
# FastAPI app
app = FastAPI(title="Gemini LLM API", version="1.0.0")
...
async def chat_with_gemini(request: ChatRequest):
    try:
        ...
        print(f"Gemini - Received conversation history: {len(conversation_history)} messages")
        print(f"Gemini - Current prompt: {request.prompt}")
        if conversation_history:
            print(f"Gemini - Last context message: {conversation_history[-1]}")
...
async def improve_prompt(request: dict):
    try:
        ...
        # Use the gemini agent to improve the prompt
        improved_prompt = await gemini_agent(
